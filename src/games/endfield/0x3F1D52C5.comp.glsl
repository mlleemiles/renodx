...
void main()
{
    ...
    for( int side = 0; side < 2; side++ )
    {
		...
		imageStore(_GTAOBlurAOTermOutRT, pixCoord, vec4(aoTerm[side], textureLod(sampler2D(_GTAOMainAOTermRT, s_point_clamp_sampler), (vec2(pixCoord.xy) + vec2(0.5)) * _GTAOData._GTAOHalfScreenSize.zw, 0.0).y, aoTerm[side], aoTerm[side]));
	}
