...
#define AO_USE_BITMASK        (shader_injection.ao_bitmask != 0.0f)	// We will delete once we finalize
...
#define XE_GTAO_SCALED_BUFFER_PIXEL_SIZE ShaderVariablesGlobal._ScreenSize.zw
...
void main()
{
    ...
    vec2 _142 = (_137 + vec2(0.5)) * ShaderVariablesGlobal._ScreenSize.zw;
...
// viewspace Zs left top right bottom
    ...
    vec3 LEFT     = XeGTAO_ComputeViewspacePosition( normalizedScreenPos + vec2(-1,  0) * ShaderVariablesGlobal._ScreenSize.zw, pixLZ );
    vec3 RIGHT    = XeGTAO_ComputeViewspacePosition( normalizedScreenPos + vec2( 1,  0) * ShaderVariablesGlobal._ScreenSize.zw, pixRZ );

    // FIX: Invert the Y signs here ( -1 becomes 1, and 1 becomes -1 )
    vec3 TOP      = XeGTAO_ComputeViewspacePosition( normalizedScreenPos + vec2( 0,  1) * ShaderVariablesGlobal._ScreenSize.zw, pixTZ );
    vec3 BOTTOM   = XeGTAO_ComputeViewspacePosition( normalizedScreenPos + vec2( 0, -1) * ShaderVariablesGlobal._ScreenSize.zw, pixBZ );
    ...
    // see "Algorithm 1" in https://www.activision.com/cdn/research/Practical_Real_Time_Strategies_for_Accurate_Indirect_Occlusion_NEW%20VERSION_COLOR.pdf
    {
		...
		for( float slice = 0; slice < float(AO_DIRECTION_COUNT); slice++ )
		{
            ...
            for( float step = 0; step < float(AO_STEP_COUNT); step++ )
            {
                ...
                float SZ0 = textureLod( sampler2D(_GTAODepthMIPsFull, s_point_clamp_sampler), sampleScreenPos0, mipLevel ).x;
                float SZ1 = textureLod( sampler2D(_GTAODepthMIPsFull, s_point_clamp_sampler), sampleScreenPos1, mipLevel ).x;
				...
				if (AO_USE_BITMASK)
				{
					...
					float falloffBase1 = length( vec3(sampleDelta1.x, sampleDelta1.y, sampleDelta1.z) );
	...
	imageStore(_GTAOOutAOTermFull, ivec2(gl_GlobalInvocationID.xy), vec4(visibility));
}
