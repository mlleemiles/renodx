...
void main()
{
    ...
    for (int _47 = -4; _47 <= 4; )
    {
        _44 += textureLod(sampler2D(_DepthOfFieldOneComponentAlphaTexture, s_linear_clamp_sampler), vec2(1.0/shader_injection.render_resolution_x, 1.0/shader_injection.render_resolution_y) * (_39 + vec2(float(_47), 0.0)), 0.0).x;
        _47++;
        continue;
    }
