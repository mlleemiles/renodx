...
namespace {
#ifdef RESHADE_AO
typedef enum DOFState : uint32_t {
  DOF_NOT_DRAWN = 0,
  DOF_COC_CALC = 1, //0x01000ACB.comp.glsl, 0xEA171205.comp.glsl
  DOF_COC_DOWN_CALC = 2, //0xDAE2717C.comp.glsl
  DOF_COC_TILE_CALC = 3, //0x652DB6A2.comp.glsl
  DOF_COC_BLUR_TILE_CALC = 4, //0x92949AE1.comp.glsl
  DOF_ONE_COMP_VERT_0_CALC = 5, //0xE9013E93.comp.glsl, 0x8F92593F.comp.glsl
  DOF_ONE_COMP_HORI_0_CALC = 6, //0xB6DF2A59.comp.glsl, 0x584419C1.comp.glsl
  DOF_ONE_COMP_ALPHA_0_CALC = 7, //0x10A6B74D.comp.glsl
  DOF_ONE_COMP_ALPHA_1_CALC = 8, //0xB90436A2.comp.glsl
  DOF_ONE_COMP_VERT_1_CALC = 9, //0x940AC0AD.comp.glsl, 0x78E75F65.comp.glsl
  DOF_ONE_COMP_HORI_1_CALC = 10, //0xB5CA505A.comp.glsl, 0x2B205E8B.comp.glsl
  DOF_OUT_CALC = 11, //0x237609F8.comp.glsl
  DOF_END = 12
} DOFState;
...
struct XeGTAOMainPass {
  ...
  void update_descriptor_table(
    ...
    updates[0].descriptors = working_ao.uavs.data();
    ...
    updates[1].descriptors = working_normal.uavs.data();
...
struct XeGTAOTemporalAccumulation {
  ...
  void update_descriptor_table(
    ...
    updates[0].count = 1;
    updates[0].type = reshade::api::descriptor_type::texture_unordered_access_view;
    updates[0].descriptors = accumulated_ao.uavs.data();
...
struct XeGTAODenoise {
  ...
  void update_descriptor_table(
    ...
    updates[0].count = 1;
    updates[0].type = reshade::api::descriptor_type::texture_unordered_access_view;
    updates[0].descriptors = out_ao.uavs.data();
    ...
    updates[1].count = 1;
    updates[1].type = reshade::api::descriptor_type::texture_unordered_access_view;
    updates[1].descriptors = out_depth.uavs.data();
    ...
    updates[2].descriptors = out_normal.uavs.data();
...
struct DepthOfField {
  TextureWithViews texDepthOfFieldDownPreviousSceneColorTexture; //Image_80869, r16g16b16a16_float
  TextureWithViews texDepthOfFieldDownSceneColorRWTexture; //Image_81364, r16g16b16a16_float
  TextureWithViews texDepthOfFieldOneComponentAlphaRWTexture; //Image_99120, r32_float
  TextureWithViews texDepthOfFieldOneComponentAlphaTexture; //Image_99123, r32_float
  TextureWithViews texDepthOfFieldOneComponentHorizontalRWTexture; //Image_99126, r16g16b16a16_float
  TextureWithViews texDepthOfFieldOneComponentVerticalRWTexture0; //Image_99132, r16g16b16a16_float
  TextureWithViews texDepthOfFieldOneComponentVerticalRWTexture1; //Image_99129, r16g16b16a16_float
  TextureWithViews texDepthOfFieldDownCoCRWTexture; //Image_99141, r16g16_float
  ...
  void create_resources(
    ...
    reshade::api::resource_desc desc_r16g16b16a16_float(
        reshade::api::resource_type::texture_2d,
        width, height, 1, 1,
    ...
    reshade::api::resource_desc desc_r16g16_float(
        reshade::api::resource_type::texture_2d,
        width, height, 1, 1,
    ...
    reshade::api::resource_desc desc_r32_float(
        reshade::api::resource_type::texture_2d,
        width, height, 1, 1,
        reshade::api::format::r32_float,
        1,
    ...
    // Create textures
    device->create_resource(desc_r16g16b16a16_float, nullptr, desc_r16g16b16a16_float.usage, &texDepthOfFieldDownPreviousSceneColorTexture.texture);
    device->create_resource(desc_r16g16b16a16_float, nullptr, desc_r16g16b16a16_float.usage, &texDepthOfFieldDownSceneColorRWTexture.texture);
    device->create_resource(desc_r16g16b16a16_float, nullptr, desc_r16g16b16a16_float.usage, &texDepthOfFieldOneComponentHorizontalRWTexture.texture);
    device->create_resource(desc_r16g16b16a16_float, nullptr, desc_r16g16b16a16_float.usage, &texDepthOfFieldOneComponentVerticalRWTexture0.texture);
    device->create_resource(desc_r16g16b16a16_float, nullptr, desc_r16g16b16a16_float.usage, &texDepthOfFieldOneComponentVerticalRWTexture1.texture);
    device->create_resource(desc_r16g16_float, nullptr, desc_r16g16_float.usage, &texDepthOfFieldDownCoCRWTexture.texture);
    device->create_resource(desc_r32_float, nullptr, desc_r32_float.usage, &texDepthOfFieldOneComponentAlphaRWTexture.texture);
    device->create_resource(desc_r32_float, nullptr, desc_r32_float.usage, &texDepthOfFieldOneComponentAlphaTexture.texture);

    // Helper lambda to create views
    auto create_uav = [&](TextureWithViews& tex, reshade::api::format format) {
      tex.uavs.resize(1);
      reshade::api::resource_view_desc view{};
      view.type = reshade::api::resource_view_type::texture_2d;
      view.format = format;
      view.texture.first_level = 0;
      view.texture.level_count = 1;
      device->create_resource_view(tex.texture, reshade::api::resource_usage::unordered_access, view, tex.uavs.data());
    };

    auto create_srv = [&](TextureWithViews& tex, reshade::api::format format) {
      tex.srvs.resize(1);
      reshade::api::resource_view_desc view{};
      view.type = reshade::api::resource_view_type::texture_2d;
      view.format = format;
      view.texture.first_level = 0;
      view.texture.level_count = 1;
      device->create_resource_view(tex.texture, reshade::api::resource_usage::shader_resource, view, tex.srvs.data());
    };

    // Create UAVs
    create_uav(texDepthOfFieldDownPreviousSceneColorTexture, reshade::api::format::r16g16b16a16_float);
    create_uav(texDepthOfFieldDownSceneColorRWTexture, reshade::api::format::r16g16b16a16_float);
    create_uav(texDepthOfFieldOneComponentAlphaRWTexture, reshade::api::format::r32_float);
    create_uav(texDepthOfFieldOneComponentAlphaTexture, reshade::api::format::r32_float);
    create_uav(texDepthOfFieldOneComponentHorizontalRWTexture, reshade::api::format::r16g16b16a16_float);
    create_uav(texDepthOfFieldOneComponentVerticalRWTexture0, reshade::api::format::r16g16b16a16_float);
    create_uav(texDepthOfFieldOneComponentVerticalRWTexture1, reshade::api::format::r16g16b16a16_float);
    create_uav(texDepthOfFieldDownCoCRWTexture, reshade::api::format::r16g16_float);

    // Create SRVs
    create_srv(texDepthOfFieldDownPreviousSceneColorTexture, reshade::api::format::r16g16b16a16_float);
    create_srv(texDepthOfFieldDownSceneColorRWTexture, reshade::api::format::r16g16b16a16_float);
    create_srv(texDepthOfFieldOneComponentAlphaRWTexture, reshade::api::format::r32_float);
    create_srv(texDepthOfFieldOneComponentAlphaTexture, reshade::api::format::r32_float);
    create_srv(texDepthOfFieldOneComponentHorizontalRWTexture, reshade::api::format::r16g16b16a16_float);
    create_srv(texDepthOfFieldOneComponentVerticalRWTexture0, reshade::api::format::r16g16b16a16_float);
    create_srv(texDepthOfFieldOneComponentVerticalRWTexture1, reshade::api::format::r16g16b16a16_float);
    create_srv(texDepthOfFieldDownCoCRWTexture, reshade::api::format::r16g16_float);
  }

  void destroy_resources(reshade::api::device* device) {
    auto destroy = [&](TextureWithViews& tex) {
      for (auto uav : tex.uavs) device->destroy_resource_view(uav);
      for (auto srv : tex.srvs) device->destroy_resource_view(srv);
      if (tex.texture.handle) device->destroy_resource(tex.texture);
      tex.uavs.clear();
      tex.srvs.clear();
      tex.texture = {};
    };

    destroy(texDepthOfFieldDownPreviousSceneColorTexture);
    destroy(texDepthOfFieldDownSceneColorRWTexture);
    destroy(texDepthOfFieldOneComponentAlphaRWTexture);
    destroy(texDepthOfFieldOneComponentAlphaTexture);
    destroy(texDepthOfFieldOneComponentHorizontalRWTexture);
    destroy(texDepthOfFieldOneComponentVerticalRWTexture0);
    destroy(texDepthOfFieldOneComponentVerticalRWTexture1);
    destroy(texDepthOfFieldDownCoCRWTexture);
  }

  void update_descriptor_table(reshade::api::device* device, DOFState state, const reshade::api::descriptor_table& table) {
    // Helper lambda to set up a single descriptor update entry
    auto set_update = [&](reshade::api::descriptor_table_update& update,
                          uint32_t binding,
    ...
    // Helper lambda to safely get view pointer (returns nullptr if empty)
    auto get_view = [](const std::vector<reshade::api::resource_view>& views) -> const reshade::api::resource_view* {
    ...
    switch (state) {
      case DOF_COC_DOWN_CALC: {
        ...
        reshade::api::descriptor_table_update updates[count]{};

        set_update(updates[0], 0, reshade::api::descriptor_type::texture_unordered_access_view,
                   get_view(texDepthOfFieldDownCoCRWTexture.uavs));
        set_update(updates[1], 1, reshade::api::descriptor_type::texture_unordered_access_view,
                   get_view(texDepthOfFieldDownSceneColorRWTexture.uavs));
        set_update(updates[2], 4, reshade::api::descriptor_type::texture_shader_resource_view,
                   get_view(texDepthOfFieldDownPreviousSceneColorTexture.srvs));

        device->update_descriptor_tables(count, updates);
        return;
      }
      ...
      case DOF_COC_TILE_CALC: {
        ...
        set_update(updates[0], 1, reshade::api::descriptor_type::texture_shader_resource_view,
                   get_view(texDepthOfFieldDownCoCRWTexture.srvs));
      ...
      case DOF_ONE_COMP_VERT_0_CALC: {
        ...
        set_update(updates[0], 0, reshade::api::descriptor_type::texture_unordered_access_view,
                   get_view(texDepthOfFieldOneComponentVerticalRWTexture0.uavs));
        set_update(updates[1], 1, reshade::api::descriptor_type::texture_unordered_access_view,
                   get_view(texDepthOfFieldOneComponentVerticalRWTexture1.uavs));
        set_update(updates[2], 2, reshade::api::descriptor_type::texture_unordered_access_view,
                   get_view(texDepthOfFieldOneComponentAlphaTexture.uavs));
        set_update(updates[3], 4, reshade::api::descriptor_type::texture_shader_resource_view,
                   get_view(texDepthOfFieldDownSceneColorRWTexture.srvs));
      ...
      case DOF_ONE_COMP_HORI_0_CALC: {
        ...
        set_update(updates[0], 0, reshade::api::descriptor_type::texture_unordered_access_view,
                   get_view(texDepthOfFieldOneComponentHorizontalRWTexture.uavs));
        set_update(updates[1], 1, reshade::api::descriptor_type::texture_unordered_access_view,
                   get_view(texDepthOfFieldOneComponentAlphaRWTexture.uavs));
        set_update(updates[2], 3, reshade::api::descriptor_type::texture_shader_resource_view,
                   get_view(texDepthOfFieldOneComponentVerticalRWTexture0.srvs));
        set_update(updates[3], 4, reshade::api::descriptor_type::texture_shader_resource_view,
                   get_view(texDepthOfFieldOneComponentVerticalRWTexture1.srvs));
        set_update(updates[4], 5, reshade::api::descriptor_type::texture_shader_resource_view,
                   get_view(texDepthOfFieldOneComponentAlphaTexture.srvs));
      ...
      case DOF_ONE_COMP_ALPHA_0_CALC: {
        ...
        set_update(updates[0], 0, reshade::api::descriptor_type::texture_unordered_access_view,
                   get_view(texDepthOfFieldOneComponentAlphaTexture.uavs));
        set_update(updates[1], 1, reshade::api::descriptor_type::texture_shader_resource_view,
                   get_view(texDepthOfFieldOneComponentAlphaRWTexture.srvs));
      ...
      case DOF_ONE_COMP_VERT_1_CALC: {
        ...
        set_update(updates[1], 1, reshade::api::descriptor_type::texture_unordered_access_view,
                   get_view(texDepthOfFieldOneComponentVerticalRWTexture1.uavs));
        set_update(updates[2], 2, reshade::api::descriptor_type::texture_shader_resource_view,
                   get_view(texDepthOfFieldDownCoCRWTexture.srvs));
        set_update(updates[3], 3, reshade::api::descriptor_type::texture_shader_resource_view,
                   get_view(texDepthOfFieldDownSceneColorRWTexture.srvs));
      ...
      case DOF_ONE_COMP_HORI_1_CALC: {
        ...
        set_update(updates[0], 0, reshade::api::descriptor_type::texture_unordered_access_view,
                   get_view(texDepthOfFieldDownPreviousSceneColorTexture.uavs));
        set_update(updates[1], 1, reshade::api::descriptor_type::texture_shader_resource_view,
                   get_view(texDepthOfFieldDownCoCRWTexture.srvs));
        set_update(updates[2], 2, reshade::api::descriptor_type::texture_shader_resource_view,
                   get_view(texDepthOfFieldDownSceneColorRWTexture.srvs));
        set_update(updates[3], 3, reshade::api::descriptor_type::texture_shader_resource_view,
                   get_view(texDepthOfFieldOneComponentVerticalRWTexture0.srvs));
        set_update(updates[4], 4, reshade::api::descriptor_type::texture_shader_resource_view,
                   get_view(texDepthOfFieldOneComponentVerticalRWTexture1.srvs));
      ...
      case DOF_OUT_CALC: {
        ...
        set_update(updates[0], 3, reshade::api::descriptor_type::texture_shader_resource_view,
                   get_view(texDepthOfFieldOneComponentHorizontalRWTexture.srvs));
        set_update(updates[1], 4, reshade::api::descriptor_type::texture_shader_resource_view,
                   get_view(texDepthOfFieldDownPreviousSceneColorTexture.srvs));
        set_update(updates[2], 5, reshade::api::descriptor_type::texture_shader_resource_view,
                   get_view(texDepthOfFieldOneComponentAlphaRWTexture.srvs));
...
;
  ...
  reshade::api::resource_view srvDepthOfFieldDownMotionVectorTexture;
  reshade::api::resource_view srvSceneDepthTexture; // not used in later pipelines

  reshade::api::resource_view srvDepthOfFieldCoCRWTexture;
  reshade::api::resource_view uavDepthOfFieldCoCRWTexture;

  reshade::api::resource_view uavDepthOfFieldOutTexture;

  reshade::api::resource_view srvDepthOfFieldBlurTileCoCRWTexture;
  reshade::api::resource_view uavDepthOfFieldBlurTileCoCRWTexture;

  reshade::api::resource_view srvDepthOfFieldTileCoCRWTexture;
  reshade::api::resource_view uavDepthOfFieldTileCoCRWTexture;

  reshade::api::sampler s_point_clamp_sampler;
  reshade::api::sampler s_linear_clamp_sampler;
  */
};
...
struct __declspec(uuid("595827c4-19b2-4300-af4d-c6802d6c7636")) DeviceData {
  ...
  reshade::api::pipeline_layout current_pipeline_layout;
  ...
  void destroy(reshade::api::device* device) {
    ...
    dof.destroy_resources(device);
  }
};
#endif

DOFState dof_state = DOF_NOT_DRAWN;
int16_t screen_width = 0;
int16_t screen_height = 0;
uint32_t render_width = 0;
uint32_t render_height = 0;
uint32_t fullscreen_dispatch_size[2] = {0, 0};

const uint32_t shader_hahs_list_dof[] = {
    0x237609F8,
};
...
bool dof_resource_need_recreate = false;
bool dof_lowres = false;

float setting_present_mode = 1.0f;
  ...
  if (!render_res_confirmed) {
    ...
    if (current_rtv.texture.format == reshade::api::format::r10g10b10a2_typeless) {
      ...
      if (current_rtv.texture.width != render_width || current_rtv.texture.height != render_height) {
        ...
        dof_resource_need_recreate = true;
      }
      render_width = current_rtv.texture.width;
      render_height = current_rtv.texture.height;

      shader_injection.render_resolution_x = static_cast<float>(render_width);
      shader_injection.render_resolution_y = static_cast<float>(render_height);

      fullscreen_dispatch_size[0] = (render_width + 8 - 1) / 8;
      fullscreen_dispatch_size[1] = (render_height + 8 - 1) / 8;
...
bool DispatchDoFComputePass(
  ...
  auto* data = renodx::utils::data::Get<DeviceData>(device);
  if (data == nullptr) {
    reshade::log::message(reshade::log::level::warning, "DoF: Failed to get DeviceData.");
    return false;
  }

  auto* shader_state = renodx::utils::shader::GetCurrentState(cmd_list);
  if (shader_state == nullptr) {
    reshade::log::message(reshade::log::level::warning, "DoF: Failed to get current shader state.");
    return false;
  }

  constexpr int COMPUTE_INDEX = renodx::utils::shader::COMPUTE_INDEX;

  auto& state = shader_state->stage_states[COMPUTE_INDEX];
  auto* details = state.pipeline_details;

  if (details == nullptr || details->layout_data == nullptr) {
    reshade::log::message(reshade::log::level::warning, "DoF: Invalid pipeline details or layout data.");
    return false;
  }

  auto& dof = data->dof;
  auto* pipeline_layout_data = details->layout_data;

  dof.update_descriptor_table(device, dof_state, data->current_descriptor_tables[0]);

  cmd_list->bind_pipeline(reshade::api::pipeline_stage::all_compute, details->pipeline);
  ...
  // Safety check for injection_index bounds
  if (pipeline_layout_data->injection_index >= pipeline_layout_data->params.size()) {
    reshade::log::message(reshade::log::level::error, "DoF: Injection index out of bounds.");
    return false;
  }

  auto pc_param = pipeline_layout_data->params[pipeline_layout_data->injection_index];

  renodx::utils::constants::PushShaderInjections(
      cmd_list,
      pipeline_layout_data->injection_layout,
      pipeline_layout_data->injection_index,
      true,
      {renodx::mods::shader::shader_injection, renodx::mods::shader::shader_injection_size},
      renodx::mods::shader::constant_buffer_offset != 0
        ? renodx::mods::shader::constant_buffer_offset
        : pipeline_layout_data->injection_constant_buffer_offset,
      renodx::mods::shader::resource_tag_float,
      -1,
...
renodx::mods::shader::CustomShaders custom_shaders = {
    ...
    {0xCC3559D9, {
         ...
         .on_drawn = [](auto* cmd_list) {
           shader_injection.ui_disable_flag = 0.0f;
           return true;
         },
     },
    },
    ...
    //DOF dispatches
    //Can move to .on_draw without modifying mods\shader.hpp, will test later
    ...
    {0x01000ACB, {
         ...
         .on_draw = [](auto* cmd_list) {
           ...
           if (dof_resource_need_recreate) {
             data->dof.destroy_resources(device);
             data->dof.create_resources(device, render_width, render_height);
             dof_resource_need_recreate = false;
           } else if (data->dof.texDepthOfFieldDownPreviousSceneColorTexture.texture.handle == 0) {
             data->dof.create_resources(device, render_width, render_height);
           }

           dof_state = DOF_COC_DOWN_CALC;
           return true;
         },
     },
    },
    ...
    {0xDAE2717C, {
         ...
         .on_draw = [](auto* cmd_list) {
           dof_state = DOF_COC_DOWN_CALC;
           if (!dof_lowres) {
             dof_state = DOF_COC_TILE_CALC;
             return true;
           } else {
             DispatchDoFComputePass(cmd_list, dof_state, fullscreen_dispatch_size[0], fullscreen_dispatch_size[1], 1);
             dof_state = DOF_COC_TILE_CALC;
             return false;
           }
         },
    ...
    {0x652DB6A2, {
         ...
         .on_draw = [](auto* cmd_list) {
           dof_state = DOF_COC_TILE_CALC;
           if (!dof_lowres) {
             dof_state = DOF_COC_BLUR_TILE_CALC;
             return true;
           } else {
             DispatchDoFComputePass(cmd_list, dof_state, (render_width / 8 + 8 - 1) / 8, (render_height / 8 + 8 - 1) / 8, 1);
             dof_state = DOF_COC_BLUR_TILE_CALC;
             return false;
           }
         },
    ...
    //DOF_COC_BLUR_TILE_CALC
    {0x92949AE1, {
         .crc32 = 0x92949AE1,
         //.code = __0x92949AE1,
         .on_draw = [](auto* cmd_list) {
           dof_state = DOF_COC_BLUR_TILE_CALC;
           dof_state = DOF_ONE_COMP_VERT_0_CALC;
           return true;
         },
     },
    },

    //DOF_ONE_COMP_VERT_0_CALC
    {0xE9013E93, {
         .crc32 = 0xE9013E93,
         .code = __0xE9013E93,
         .on_draw = [](auto* cmd_list) {
           dof_state = DOF_ONE_COMP_VERT_0_CALC;
           if (!dof_lowres) {
             dof_state = DOF_ONE_COMP_HORI_0_CALC;
             return true;
           } else {
             DispatchDoFComputePass(cmd_list, dof_state, fullscreen_dispatch_size[0], fullscreen_dispatch_size[1], 1);
             dof_state = DOF_ONE_COMP_HORI_0_CALC;
    ...
    {0xB6DF2A59, {
         ...
         .on_draw = [](auto* cmd_list) {
           dof_state = DOF_ONE_COMP_HORI_0_CALC;
           if (!dof_lowres) {
             dof_state = DOF_ONE_COMP_ALPHA_0_CALC;
             return true;
           } else {
             DispatchDoFComputePass(cmd_list, dof_state, fullscreen_dispatch_size[0], fullscreen_dispatch_size[1], 1);
             dof_state = DOF_ONE_COMP_ALPHA_0_CALC;
    ...
    {0x10A6B74D, {
         ...
         .on_draw = [](auto* cmd_list) {
           dof_state = DOF_ONE_COMP_ALPHA_0_CALC;
           if (!dof_lowres) {
             dof_state = DOF_ONE_COMP_ALPHA_1_CALC;
             return true;
           } else {
             DispatchDoFComputePass(cmd_list, dof_state, fullscreen_dispatch_size[0], fullscreen_dispatch_size[1], 1);
             dof_state = DOF_ONE_COMP_ALPHA_1_CALC;
    ...
    {0xB90436A2, {
         ...
         .on_draw = [](auto* cmd_list) {
           dof_state = DOF_ONE_COMP_ALPHA_1_CALC;
           if (!dof_lowres) {
             dof_state = DOF_ONE_COMP_VERT_1_CALC;
             return true;
           } else {
             DispatchDoFComputePass(cmd_list, dof_state, fullscreen_dispatch_size[0], fullscreen_dispatch_size[1], 1);
             dof_state = DOF_ONE_COMP_VERT_1_CALC;
    ...
    {0x940AC0AD, {
         ...
         .on_draw = [](auto* cmd_list) {
           dof_state = DOF_ONE_COMP_VERT_1_CALC;
           if (!dof_lowres) {
             dof_state = DOF_ONE_COMP_HORI_1_CALC;
             return true;
           } else {
             DispatchDoFComputePass(cmd_list, dof_state, fullscreen_dispatch_size[0], fullscreen_dispatch_size[1], 1);
             dof_state = DOF_ONE_COMP_HORI_1_CALC;
...
renodx::utils::settings::Settings settings = {
    ...
    new renodx::utils::settings::Setting{
        .key = "PresentMode",
        .binding = &setting_present_mode,
        .value_type = renodx::utils::settings::SettingValueType::INTEGER,
        .default_value = 0.f,
        .label = "Present Mode",
        .tooltip = "Change vsync present mode",
        .labels = {"MAILBOX", "FIFO", "FIFO_RELAXED", "FIFO_LATEST_READY"},
        .min = 0.f,
        .max = 3.f,
    },
    new renodx::utils::settings::Setting{
...
static bool OnCreateSwapchain(reshade::api::device_api device_api, reshade::api::swapchain_desc& desc, void* hwnd) {
  ...
  if (!renodx::mods::swapchain::ShouldModifySwapchain(static_cast<HWND>(hwnd), device_api)) {
    std::stringstream s;
  ...
  auto old_sync_interval = desc.sync_interval;
  ...
  s << ", fullscreen_state:";
  s << "0x" << std::hex << old_fullscreen_state << std::dec;
  s << ", fullscreen_refresh_rate:" << old_fullscreen_refresh_rate << std::dec;
  s << ", sync_interval:" << old_sync_interval << std::dec;
  s << ", buffers:";
  s << old_buffer_count;
  ...
  if (old_present_mode != static_cast<uint32_t>(VK_PRESENT_MODE_IMMEDIATE_KHR)) {
    switch (static_cast<int>(setting_present_mode)) {
      case 0: desc.present_mode = static_cast<uint32_t>(VK_PRESENT_MODE_MAILBOX_KHR);
        break;
      case 1: desc.present_mode = static_cast<uint32_t>(VK_PRESENT_MODE_FIFO_KHR);
        break;
      case 2: desc.present_mode = static_cast<uint32_t>(VK_PRESENT_MODE_FIFO_RELAXED_KHR);
        break;
      case 3: desc.present_mode = static_cast<uint32_t>(VK_PRESENT_MODE_FIFO_LATEST_READY_KHR);
        break;
...
void OnBindDescriptorTables(
  ...
  if (/*!gtao_has_drawn && gtao_will_draw && */stages == reshade::api::shader_stage::compute) {
    auto* device = cmd_list->get_device();
    auto* data = renodx::utils::data::Get<DeviceData>(device);
    if (data == nullptr) return;

    data->current_pipeline_layout = layout;
...
void OnBarrier(reshade::api::command_list* cmd_list, uint32_t count, const reshade::api::resource* resources, const reshade::api::resource_usage* old_states, const reshade::api::resource_usage* new_states) {
  if (dof_state == DOF_COC_DOWN_CALC) {
    auto* device = cmd_list->get_device();
    if (device == nullptr) return;
    for (uint32_t i = 0; i < count; ++i) {
      auto resource_desc = device->get_resource_desc(resources[i]);
      /*
      {
        std::stringstream s;
        s << "[DOF Pass Resource " << i << "] ";
        s << "Format: " << resource_desc.texture.format;
        s << ", Width: " << resource_desc.texture.width << "x" << resource_desc.texture.height;
        reshade::log::message(reshade::log::level::info, s.str().c_str());
      }
      */
      if (resource_desc.texture.format == reshade::api::format::r16g16_float || resource_desc.texture.format == reshade::api::format::r16g16_typeless) {
        if (resource_desc.texture.width != render_width) {
          if (!dof_lowres) {
            reshade::log::message(reshade::log::level::info, "DOF: Current DOF is low res.");
          }
          dof_lowres = true;
          dof_state = DOF_NOT_DRAWN; //prevent repeated run if its not changed later in the pipeline
          return;
        }
      }
    }
...
BOOL APIENTRY DllMain(HMODULE h_module, DWORD fdw_reason, LPVOID lpv_reserved) {
  ...
  switch (fdw_reason) {
    case DLL_PROCESS_ATTACH: if (!reshade::register_addon(h_module)) return FALSE;
      ...
      break;
    case DLL_PROCESS_DETACH: reshade::unregister_event<reshade::addon_event::init_swapchain>(OnInitSwapchain);
