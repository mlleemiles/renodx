...
layout(set = 2, binding = 0, r32f) uniform writeonly image2D _GTAODepthMipFull0;
layout(set = 2, binding = 1, r32f) uniform writeonly image2D _GTAODepthMipFull1;
layout(set = 2, binding = 2, r32f) uniform writeonly image2D _GTAODepthMipFull2;
layout(set = 2, binding = 3, r32f) uniform writeonly image2D _GTAODepthMipFull3;
layout(set = 2, binding = 4, r32f) uniform writeonly image2D _GTAODepthMipFull4;
...
{	
    ...
    vec4 _91 = textureGatherOffset(sampler2D(_SceneDepthTexture, s_point_clamp_sampler), vec2(_82) * ShaderVariablesGlobal._ScreenSize.zw, ivec2(1));
    ...
    imageStore(_GTAODepthMipFull0, ivec2(_82), vec4(_100));
    imageStore(_GTAODepthMipFull0, ivec2(_82 + uvec2(1u, 0u)), vec4(_105));
    imageStore(_GTAODepthMipFull0, ivec2(_82 + uvec2(0u, 1u)), vec4(_110));
    imageStore(_GTAODepthMipFull0, ivec2(_82 + uvec2(1u)), vec4(_115));
    ...
    imageStore(_GTAODepthMipFull1, ivec2(gl_GlobalInvocationID.xy), vec4(_166));
    ...
    if (all(equal((gl_LocalInvocationID.xy % uvec2(2u)), uvec2(0u))))
    {
        ...
        imageStore(_GTAODepthMipFull2, ivec2(gl_GlobalInvocationID.xy / uvec2(2u)), vec4(_214));
    ...
    if (all(equal((gl_LocalInvocationID.xy % uvec2(4u)), uvec2(0u))))
    {
        ...
        imageStore(_GTAODepthMipFull3, ivec2(gl_GlobalInvocationID.xy / uvec2(4u)), vec4(_260));
    ...
    if (all(equal((gl_LocalInvocationID.xy % uvec2(8u)), uvec2(0u))))
    {
        ...
        imageStore(_GTAODepthMipFull4, ivec2(gl_GlobalInvocationID.xy / uvec2(8u)), vec4(((((_283 * g_scratchDepths[gl_LocalInvocationID.x][gl_LocalInvocationID.y]) + (_287 * g_scratchDepths[_269][gl_LocalInvocationID.y])) + (_291 * g_scratchDepths[gl_LocalInvocationID.x][_272])) + (_295 * g_scratchDepths[_269][_272])) / (((_283 + _287) + _291) + _295)));
    }
}
